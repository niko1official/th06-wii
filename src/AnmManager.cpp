...
static const PixelDataType g_TextureFormatTypeMapping[6] = {static_cast<PixelDataType>(0),
...
{
    if (this->surfaceTextureHandles[surfaceIdx])
    {
        if (this->currentTextureHandle.id == this->surfaceTextureHandles[surfaceIdx].id)
            this->SetCurrentTexture(0);
        g_GfxBackend->DeleteTexture(this->surfaceTextureHandles[surfaceIdx]);
        this->surfaceTextureHandles[surfaceIdx] = GfxTextureHandle();
    }
    if (this->surfaces[surfaceIdx] != NULL)
    {
...
void AnmManager::ApplySurfaceToColorBuffer(SDL_Surface *src, const SDL_Rect &srcRect, const SDL_Rect &dstRect)
{
    ...
    for (i32 idx = 0; idx < ARRAY_SIZE_SIGNED(this->surfaces); idx++)
    {
        ...
        {
            cachedSurfaceIdx = idx;
            break;
        }
    }

    if (cachedSurfaceIdx >= 0 && this->surfaceTextureHandles[cachedSurfaceIdx])
    {
        this->SetCurrentTexture(this->surfaceTextureHandles[cachedSurfaceIdx]);
    }
    else
    {
        CreateTextureObject();
        u8 *surfaceData = ExtractSurfacePixels(src, 3);
        u8 *paddedSurfaceData = new u8[textureWidth * textureHeight * 3];
        std::memset(paddedSurfaceData, 0, textureWidth * textureHeight * 3);
        for (i32 y = 0; y < src->h; y++)
        {
            std::memcpy(paddedSurfaceData + y * textureWidth * 3, surfaceData + y * src->w * 3,
                        src->w * 3);
        }

        g_GfxBackend->SetTextureImage(textureWidth, textureHeight, PIXEL_RGB, PIXEL_UNSIGNED_BYTE,
                                      paddedSurfaceData);
        delete[] surfaceData;
        delete[] paddedSurfaceData;

        if (cachedSurfaceIdx >= 0)
            this->surfaceTextureHandles[cachedSurfaceIdx] = this->currentTextureHandle;
    }
